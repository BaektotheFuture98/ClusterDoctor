...
시각을 인자로 받는 이유: ``@timestamp``를 본문에 UTC 문자열로 박아 두면
과거 구간을 재현할 때마다 그 줄을 고쳐야 하고, ``Z``가 UTC라는 것을 잊으면
의도한 KST 시각에서 정확히 9시간 어긋난 구간을 분석하게 된다 — 조회가
성공하고 결과만 틀리므로 어디에서도 드러나지 않는 종류의 오류다. ``--at``은 오프셋이 없으면 KST로 읽고, ``--dry-run``이
