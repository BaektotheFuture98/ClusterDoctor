...
def build_diagnosis_subagent(
    ...
    **호출 스레드 주의.** 반환되는 runnable은 동기이고 내부에서 수 분 블로킹될
    ...
    def _run(agent_state: dict[str, Any]) -> dict[str, Any]:
        ...
        delegation = _DiagnosisSession(window, request)
...
def _unresolved_gaps(
    seams: DiagnosisSeams, delegation: _DiagnosisSession
) -> tuple[TimeRange, ...]:
    """근거를 확보하지 못한 시간 범위.
...
def _summary_without_report(delegation: _DiagnosisSession) -> str:
