"""Hierarchical Map-Reduce Log Triage 그래프.

    chunk_by_minute
         │
         ├── Send ──> map_minute (분 1)
         ├── Send ──> map_minute (분 2)      팬아웃
         └── Send ──> map_minute (분 N)
                          │
...
def run_triage(
    ...
    *,
    ...
    builder.add_node(_CHUNK, make_chunk_by_minute())
    ...
    builder.add_edge(START, _CHUNK)
    builder.add_conditional_edges(_CHUNK, dispatch, [_MAP, _REDUCE])
